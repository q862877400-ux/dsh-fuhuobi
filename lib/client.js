      ...
      return h('li', { className: 'gdb-card' + (open ? ' gdb-card-open' : '') },
        ...
        open
          ? h('div', { className: 'gdb-card-body' },
              ...
              h('p', { className: 'gdb-hint' }, '完整管理（查看复活币 / 一键复活 / 手动存币）在 设置 → 复活币口袋 页面。'),
            )
          : null,
      )
    }
    ...
    function CrashOverlay({ message }) {
      ...
      const closeBtn = h('button', {
        ...
        style: {
          ...
          fontSize: '22px', color: 'rgba(255,255,255,.85)', lineHeight: '1',
          padding: '6px', zIndex: 2,
        },
        'aria-label': '关闭',
      ...
      return h('div', {
        ...
        closeBtn,
        h('div', { style: { maxWidth: '560px', padding: '32px', textAlign: 'center', color: '#e8e2d8' } },
          h('div', { style: { fontSize: '15px', letterSpacing: '4px', color: '#8a8078', marginBottom: '20px' } }, '你 死 了'),
          h('div', { style: { fontSize: '17px', lineHeight: '1.8', marginBottom: '28px', color: '#d8cfc2' } },
            'DSH 因插件崩溃而倒下。', h('br'),
            '别怕，复活币还在。', h('br'),
            h('span', { style: { color: '#8a8078', fontSize: '13px' } }, '（回滚到上一次成功启动的状态）')),
          h('button', {
            disabled: working || done,
            onClick: doRevive,
            style: {
              fontSize: '17px', padding: '14px 44px', cursor: working ? 'wait' : 'pointer',
              background: 'linear-gradient(180deg,#6b4a2f,#4a3220)',
              border: '1px solid #8a6a45', borderRadius: '8px', color: '#ffe9c9',
              letterSpacing: '3px', fontFamily: 'inherit',
            },
          }, done ? '已恢复，即将刷新…' : working ? '复活中…' : '🔥 使用复活币'),
          err ? h('div', { style: { marginTop: '16px', color: '#c96b5b', fontSize: '13px' } }, err) : null,
          h('div', { style: { marginTop: '28px', fontSize: '12px', color: '#6f665c', lineHeight: '1.7' } },
