...
`
    ...
    function injectGuardedRestartButton() {
      try {
        ...
        // ── CSS（独立类，不干扰硬性重启）──
        if (!document.getElementById('fuhuobi-rst-style')) {
          const style = document.createElement('style')
          style.id = 'fuhuobi-rst-style'
          style.textContent = [
            '.fuhuobi-rst{position:absolute!important;top:50%!important;transform:translateY(-50%)!important;width:28px;height:28px;color:color-mix(in srgb,var(--dsw-alias-label-secondary) 55%,transparent);background:transparent;border:none;border-radius:50%;padding:0;display:inline-flex;align-items:center;justify-content:center;cursor:pointer;transition:color .15s ease,background .15s ease;z-index:10}',
        ...
        // 仅当设置区还是 static 时才设为 relative（与 niao 的 nio-rst-area 行为一致，
        // 让我们的 absolute 按钮能正确对齐；若已是 relative 绝不动，避免影响硬性重启）。
        if (window.getComputedStyle(area).position === 'static') {
          area.style.position = 'relative'
        }
        // 有硬性重启 → 让位放左边(42px)；没有 → 站到它的位置(8px)。绝不改硬性重启。
        const nioBtn = area.querySelector('[data-nio-rst]')
        btn.style.right = (nioBtn ? '42px' : '8px')
        if (window.console) console.log('[dsh-fuhuobi] 存复活币按钮已注入，right=' + btn.style.right)
      } catch { /* best effort */ }
    }
