...
`
    ...
    function BackupsSection() {
      ...
      return React.createElement('div', { className: 'gdb-wrap' },
        ...
        React.createElement('div', { className: 'gdb-hint', style: { display: 'flex', flexDirection: 'column', gap: '8px', border: '1px solid var(--dsw-alias-border-l2)', borderRadius: '8px', padding: '10px' } },
          ...
          // 无 Python 提示
          !hasPythonPil
            ? React.createElement('div', { style: { fontSize: '11px', color: 'var(--dsw-alias-state-warn-primary)', marginTop: '2px' } },
    ...
    // ── 黑屏崩溃覆盖层 ──
    // 根组件渲染崩溃时，立即渲染一个全屏覆盖层：魂系 DIE 通告 + 右上角 ✕
    // 关闭按钮。这里只做「告知 + 指路」，不提供一键复活——恢复统一走桌面
    // 或 DSH 根目录的「DSH复活币X1」（一条可靠通路胜过两条半坏的）。
    // ✕ 只关闭覆盖层（不恢复崩溃的根组件），方便用户查看控制台/原页面报错。
    function CrashOverlay({ message }) {
      ...
      return h('div', {
        ...
        h('div', { style: { maxWidth: '840px', padding: '48px', textAlign: 'center', color: '#e8e2d8' } },
          ...
          h('div', { style: { fontSize: '21px', color: '#ffe9c9', lineHeight: '1.9' } },
            '请到桌面双击「DSH复活币X1」恢复；', h('br'),
            '网页打不开时，也可到 DSH 根目录双击同一文件。'),
