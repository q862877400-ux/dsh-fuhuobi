...
`
    ...
    // ── 守护重启按钮：注入到设置区，与「硬性重启」并排（尺寸/位置完全一致）──
    // 不修改任何其他插件的代码：独立 DOM 注入，复刻 niao-quick-open 的
    // nio-rst 按钮结构（绝对定位 28x28 圆形 + hover tooltip），唯一区别是
    // 图标用盾牌 + tooltip 文字「守护重启」。点它调本插件的 guarded-restart。
    const GUARD_RST_TIP = '守护重启'
    const GUARD_RST_SVG = [
      // 盾牌图标（守护意义）
      '<path d="M12 2l8 3.5v5.2c0 4.6-3.2 8.6-8 10.3-4.8-1.7-8-5.7-8-10.3V5.5z" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>',
      // 盾牌中间的勾（守护成功）
      '<path d="M8.5 12l2.5 2.5 4.5-4.5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>',
    ].join('')
    ...
    function injectGuardedRestartButton() {
      try {
        if (typeof document === 'undefined') return
        if (document.querySelector('[data-fuhuobi-rst]')) return
        const bySlot = document.querySelector('[data-slot="sidebar.settings"]')
        const byClass = document.querySelector('[class*="settingsArea"]')
        const area = bySlot || byClass
        if (!area) {
          setTimeout(injectGuardedRestartButton, 1500)
          return
        }
        if (area.closest('[class*="collapsed"]')) return
        // 给设置区标记为相对定位容器（复刻 nio-rst-area），以便绝对定位按钮
        area.classList.add('fuhuobi-rst-area')
        ...
        btn.addEventListener('click', (e) => {
          ...
          btn.setAttribute('aria-label', '守护重启中…')
          btn.classList.add('fuhuobi-rst-busy')
          btn.disabled = true
          fetch('/fuhuobi/api/guarded-restart', { method: 'POST' })
            .then(() => {})
            .catch(() => { btn.setAttribute('aria-label', prevLabel); btn.classList.remove('fuhuobi-rst-busy'); btn.disabled = false })
          setTimeout(() => {
            if (!document.hidden) {
              fetch('/fuhuobi/api/state', { method: 'GET' })
                .then(() => { btn.setAttribute('aria-label', prevLabel); btn.classList.remove('fuhuobi-rst-busy'); btn.disabled = false })
                .catch(() => { /* 服务已重启 */ })
            }
          }, 30000)
        })
        // 复刻 nio-rst CSS（圆形 28x28、绝对定位），并排放在硬性重启左边（right 42px = 28+8+6）
        const style = document.createElement('style')
        style.textContent = [
          '[class*="settingsArea"].fuhuobi-rst-area{position:relative}',
          '.fuhuobi-rst{position:absolute;right:42px;top:50%;transform:translateY(-50%);width:28px;height:28px;color:color-mix(in srgb,var(--dsw-alias-label-secondary) 55%,transparent);background:transparent;border:none;border-radius:50%;padding:0;display:inline-flex;align-items:center;justify-content:center;cursor:pointer;transition:color .15s ease,background .15s ease}',
          '.fuhuobi-rst:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-state-success-primary)}',
          '.fuhuobi-rst svg{display:block;width:15px;height:15px}',
          '.fuhuobi-rst-busy{opacity:.6;cursor:wait}',
          '.fuhuobi-rst-tip{position:absolute;bottom:calc(100% + 8px);left:50%;transform:translateX(-50%);white-space:nowrap;background:var(--dsw-alias-tooltip-bg);color:#f2f2f2;border:1px solid rgba(255,255,255,0.12);border-radius:6px;padding:4px 8px;font-size:11px;line-height:15px;pointer-events:none;opacity:0;transition:opacity .12s ease;z-index:2147483001;font-family:-apple-system,BlinkMacSystemFont,\'Segoe UI\',Roboto,sans-serif}',
          '.fuhuobi-rst:hover .fuhuobi-rst-tip{opacity:1}',
        ].join('\n')
        document.head.appendChild(style)
        area.appendChild(btn)
        if (window.console) console.log('[dsh-fuhuobi] 守护重启按钮已注入')
      } catch { /* best effort */ }
    }
    ...
    function apply(ctx) {
      ...
      // 0c. 守护重启按钮：注入到设置区，放在硬性重启按钮旁边（不修改其他插件）。
      try {
        setTimeout(injectGuardedRestartButton, 1200)
        // 侧栏可能在运行中重新渲染，周期性检查补注
        const iv = setInterval(() => {
          try {
            if (document.querySelector('[data-slot="sidebar.settings"]')) {
              injectGuardedRestartButton()
            }
          } catch { /* best effort */ }
        }, 4000)
        ctx.effect(() => () => clearInterval(iv), 'fuhuobi: guarded-restart button watcher')
      } catch { /* best effort */ }
